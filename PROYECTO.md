...
| `vercel.json` | Config de Vercel: **cache-busting** — headers `Cache-Control: no-cache, must-revalidate` para `/app.js`, `/styles.css`, `/config.js`, `/index.html` (evita que el navegador sirva una versión vieja tras un deploy). **Sí se sube** al repo. |
...
- `id` uuid · `nombre` text único · `bono_pct` numeric (**bono de depósito por casa**, ej. Vira=20, SuperPro=10; se aplica al cargar dinero, NO en las apuestas) · `tiene_cajeras` boolean (sus líneas descuentan/acreditan el saldo de la cajera, ej. Vira=true) · `permite_gratis` boolean (da "apuesta gratis", ej. Betano=true) · `mostrar_filtro` boolean (legacy: marcaba qué casinos salían en el filtro por casino del sub-tab Cajeros; **ese filtro se quitó del UI**, la columna y el checkbox de Config siguen existiendo pero hoy no filtran nada) · `creado_en`
...
- **Bottom nav fija** (estilo app): 📊 Reportes · 🥅 Partidos · 👩 Cajeras · ⚙️ Config. **Partidos y Cajeras son un poco más grandes** (más peso visual, `flex:1.35`) que Reportes y Config.
- **Entrada de montos/cuotas apta para mobile:** todos los campos de monto y cuota son `type="text" inputmode="decimal"` → el celular abre el **teclado numérico** (sin letras). Aceptan **coma o punto** como separador decimal (el teclado es-AR muestra coma): el helper `num()` normaliza `","` → `"."` antes de parsear. Así se puede escribir una cuota `1,05` o `1.05` indistintamente.
...
**Tab Cajeras** — **sub-tabs (chips) grandes** *Cajeros* / *Cuentas* (ocupan una fila, destacados sobre el resto) + una **fila de controles aparte** (`.cajeras-controles`) con el botón **⇅ Ordenar** (alterna **Actividad** ↔ **Saldo ↓**, de mayor a menor; estilo pill claramente tocable, ícono distinto al de transferir para no confundir) + **filtro por Admin** (Todos / cada admin / Sin admin; aplica a ambos sub-tabs y a los totales). *(Se quitó el filtro por Casino y el botón Ganancia del tab.)* **Cada card se colorea por admin** (igual que las apuestas): etiqueta del nombre del admin en color (● Nombre) + **borde lateral** del mismo color (Dani=rojo, Nico=azul, Kelvin=amarillo; resto por paleta). Card de resumen arriba (Saldo total del tab; en Cajeros además Total apostado). Botón **🔁 Transferir** (elige sentido cajero↔cuenta, monto; preview de comisión/bono y neto) en ambos tabs y **por card** (en cada cajero al lado de Cargar, y en cada cuenta). El **sentido y la entidad arrancan preseleccionados** según desde dónde se abre: card de cajero → *Cuenta → Cajero* con ese cajero; card de cuenta → *Cajero → Cuenta* con esa cuenta; botón global según el sub-tab (Cajeros → *Cuenta → Cajero*, Cuentas → *Cajero → Cuenta*). Las **cuentas** muestran una card simple (saldo + recibido neto + enviado + botones Transferir/Movimientos, sin toggles de apuesta). Una tarjeta por cajera con:
...
- `Bono por apuesta (catálogo de bonos)` — tabla `bonos` (listado de %) + `apuestas.bono_pct` (migración `migracion-bonos.sql`). Se elige el bono con **chips** al cargar cada apuesta y al transferir cuenta→cajero. Reemplaza el snapshot por partido (`partidos.bono_apuestas`, ahora legacy). El bono **no afecta lo apostado**, suma al profit: `ingresado × %/(100+%)`. El popup Resolver muestra el % de bono de cada apuesta.
- `Fix profit: restaurar el bono por apuesta desde el snapshot congelado` — al pasar al bono-por-apuesta, un backfill había puesto `bono_pct` = bono actual de la casa (más bajo que el congelado al resolver), y el profit histórico bajó (~$180k → −$1.6M). `migrarBonoApuestas()` (una sola vez, flag `config.bono_apuestas_migrado`) recalcula cada `apuestas.bono_pct` desde el snapshot `partidos.bono_apuestas` para que `Σ bonoDeApuesta` == snapshot exacto.
- `Bono de retiro = monto de la última carga` — el bono por saldo de retiro dejó de calcularse como `saldo × %` y pasa a ser el **monto de bono de la última carga** de cada cajera (`carga.monto × bono%/100`, ej. carga 400.000 al 20% → 80.000). El popup Resolver muestra el **desglose por cajera**.
- `Propina al retirar/transferir` — `movimientos.propina` + `transferencias.propina` (migración `migracion-propina.sql`). Campo Propina opcional en Retirar y Transferir: **sale del saldo del origen** (Retiro/transfer descuenta monto + propina) y **se resta del profit**. Informe **💵 Propinas** en Reportes (detalle + total).
- `Desactivar cajero + teclado numérico + separador decimal` — `cajeras.activo` (migración `migracion-cajera-activo.sql`): un cajero desactivado no aparece al cargar apuestas (se reactiva en Config). Campos de monto/cuota como `type=text inputmode=decimal` (teclado numérico en mobile, acepta coma o punto vía `num()`).
- `Transferencias: defaults por contexto + botón por card` — al abrir Transferir el sentido y la entidad arrancan preseleccionados (card de cajero → *Cuenta → Cajero*; card de cuenta → *Cajero → Cuenta*; botón global según el sub-tab). Botón **🔁 Transferir** en cada card de cajero (al lado de Cargar) y de cuenta.
- `Reportes: Transferencias solo cajero → cuenta` — el informe 🔁 Transferencias muestra solo las que cobran comisión (cajero → cuenta).
- `Colores por admin` — color fijo por nombre (Dani=rojo, Nico=azul, Kelvin=amarillo; resto por paleta) vía `colorAdmin`. Se aplica a cada apuesta (etiqueta ● Nombre + borde lateral) en Partidos **y** a cada card de Cajeros/Cuentas.
- `Partidos: orden por cuota + cuotas altas resaltadas` — las apuestas de un partido se ordenan por cuota (la más baja arriba); las cuotas altas se marcan en negrita y color (≥5 dorado, ≥10 naranja/púrpura).
- `Partidos: color pendiente púrpura` — el estado Pendiente pasó de ámbar a **púrpura** `#b18cff` (el ámbar chocaba con el amarillo de Kelvin).
- `Partidos: resultados posibles + combo` — `partidos.resultados` jsonb (migración `migracion-partido-resultados.sql`). Al crear el partido se ingresan ≥2 resultados posibles (lista dinámica); al cargar una apuesta el Resultado se elige de un **combo** con esos valores.
- `Cajeras: botón Ordenar + sub-tabs grandes` — sub-tabs Cajeros/Cuentas más grandes en su fila; fila de controles aparte con **⇅ Ordenar** (Actividad ↔ Saldo ↓) y filtro por Admin. Se quitó el filtro por Casino y el botón Ganancia del tab.
- `Cache-busting (vercel.json)` — headers `no-cache, must-revalidate` para app.js/styles.css/config.js/index.html, para que un deploy no quede tapado por la caché del navegador.
- `Rendimiento` — caché por render del saldo por cajera (`resumenCajera` con `_resumenCache`, se limpia al inicio de cada `render()`) y precálculo de la clave de orden, para que el tab Cajeras no recalcule N veces lo mismo.
